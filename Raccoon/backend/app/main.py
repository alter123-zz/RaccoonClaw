...
async def lifespan(app: FastAPI):
    ...
    # 初始化数据库表（开发用；生产环境应使用 Alembic 迁移）
    try:
        await init_db()
    ...
    # 启动通知服务（订阅 event bus，触发飞书/企微等渠道通知）
    if bus is not None:
        try:
            ...
            log.warning("⚠️ Notification service failed to start: %s", exc)
...
if _cors_origins:
    ...
    # 开发模式：允许所有来源（不可与 credentials=True 同时使用）
    app.add_middleware(
        CORSMiddleware,
        allow_origins=["*"],
