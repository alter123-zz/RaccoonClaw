...
async def update_remote_skill_route(body: dict):
    return update_remote_skill(body.get("agentId", "").strip(), body.get("skillName", "").strip())
...
async def repair_flow_order_route():
    return repair_flow_order()
...
# --- Agent & Scheduler State Routes ---

@router.get("/agents-status")
async def agents_status_route():
    return get_agents_status()


@router.get("/scheduler-state/{task_id}")
async def scheduler_state_route(task_id: str):
    return get_scheduler_state(task_id)


@router.get("/task-activity/{task_id}")
async def task_activity_route(task_id: str):
    return get_task_activity(task_id)


@router.get("/remote-skills-list")
async def remote_skills_list_route():
    return get_remote_skills_list()


# ── Legacy compatibility endpoints ────────────────────────────────────────────

@router.get("/model-change-log")
async def model_change_log_route():
    """GET /api/model-change-log — 读取 model 切换历史。"""
    return read_json_file("model_change_log.json", [])


@router.get("/last-result")
async def last_result_route():
    """GET /api/last-result — 读取上次 model 切换结果。"""
    return read_json_file("last_model_change_result.json", {})


_agent_activity_re = None

def _safe_agent_id(agent_id: str) -> bool:
    global _agent_activity_re
    if _agent_activity_re is None:
        import re
        _agent_activity_re = re.compile(r'^[a-zA-Z0-9_\-\u4e00-\u9fff]+$')
    return bool(_agent_activity_re.match(agent_id))


@router.get("/agent-activity/{agent_id}")
async def agent_activity_route(agent_id: str):
    """GET /api/agent-activity/{agent_id} — 读取 Agent 活跃记录。"""
    if not _safe_agent_id(agent_id):
        from fastapi import HTTPException
        raise HTTPException(status_code=400, detail="invalid agent_id")
    return {"ok": True, "agentId": agent_id, "activity": get_agent_activity(agent_id)}
