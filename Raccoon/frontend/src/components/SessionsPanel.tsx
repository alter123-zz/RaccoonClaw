import { useStore, STATE_LABEL, isTerminalState, timeAgo } from '../store';
import type { Task } from '../api';
import { useState } from 'react';
import { formatBeijingTime } from '../time';
import { extractSessionAgent, selectWorkbenchTasks } from '../workbenchSelectors';
...
export default function SessionsPanel() {
  ...
  const { sessions } = selectWorkbenchTasks(tasks, workbenchMode);

  let filtered = sessions;
  if (sessFilter === 'active') filtered = sessions.filter((t) => !isTerminalState(t.state));
  else if (sessFilter !== 'all') filtered = sessions.filter((t) => extractSessionAgent(t) === sessFilter);

  // Unique agents for filter tabs
  const agentIds = [...new Set(sessions.map(extractSessionAgent))];
    ...
    <div>
      ...
      <div className="sess-grid">
        {!filtered.length ? (
          ...
          filtered.map((t) => {
            const agent = extractSessionAgent(t);
