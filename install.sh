...
OPENCLAW_HOME="${OPENCLAW_HOME:-$REPO_DIR/.openclaw}"
OPENCLAW_CONFIG="${OPENCLAW_CONFIG_PATH:-$OPENCLAW_HOME/openclaw.json}"
GLOBAL_OPENCLAW_HOME="$HOME/.openclaw"
GLOBAL_OPENCLAW_CONFIG="$GLOBAL_OPENCLAW_HOME/openclaw.json"
DATA_PROFILE="${RACCOONCLAW_DATA_PROFILE:-clean}"
...
check_deps() {
  ...
  if [ ! -f "$OPENCLAW_CONFIG" ]; then
    if [ -f "$GLOBAL_OPENCLAW_CONFIG" ]; then
      warn "未找到本地 OpenClaw 配置，正在从 ~/.openclaw 复制一份到仓库内 runtime。"
      cp "$GLOBAL_OPENCLAW_CONFIG" "$OPENCLAW_CONFIG"
    else
      error "未找到 ${OPENCLAW_CONFIG}。请先执行：OPENCLAW_HOME=\"$OPENCLAW_HOME\" openclaw setup"
      exit 1
    fi
  fi
  log "OpenClaw 配置已找到"
...
init_data() {
  info "初始化仓库数据目录（profile=$DATA_PROFILE）"
  python3 "$REPO_DIR/scripts/seed_runtime_data.py" \
    --profile "$DATA_PROFILE" \
    --repo-dir "$REPO_DIR" \
    --openclaw-home "$OPENCLAW_HOME"
  log "data/ 与 runtime seed 已准备"
}
...
echo "下一步："
echo "  0. cp .env.example .env  # 如需自定义路径和功能开关"
