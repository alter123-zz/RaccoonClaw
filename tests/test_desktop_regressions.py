...
class DesktopRegressionTests(unittest.TestCase):
    ...
    def test_dashboard_api_source_exposes_bootstrap_routes(self) -> None:
        ...
        self.assertIn('@router.get("/public-config")', source)

    def test_frontend_source_consumes_public_feature_flags(self) -> None:
        app_source = (ROOT / "edict" / "frontend" / "src" / "App.tsx").read_text(encoding="utf-8")
        board_source = (ROOT / "edict" / "frontend" / "src" / "components" / "EdictBoard.tsx").read_text(encoding="utf-8")

        self.assertIn("loadPublicConfig()", app_source)
        self.assertIn("publicConfig.features.imChannels", app_source)
        self.assertIn("publicConfig.features.toolbox", app_source)
        self.assertIn("publicConfig.features.scheduledTasks", board_source)
