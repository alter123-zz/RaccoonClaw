...
def main():
        ...
        for ag in agents_list:
            if ag.get('id') == ag_id:
                ...
                if new_model == default_model:
                    ...
                    ag['model'] = new_model
                applied.append({'at': beijing_now().isoformat(), 'agentId': ag_id, 'oldModel': old, 'newModel': new_model})
                found = True
                break
        if not found:
    ...
    if applied:
        bak = OPENCLAW_CFG.parent / f'openclaw.json.bak.model-{beijing_now().strftime("%Y%m%d-%H%M%S")}'
