...
INTERVAL="${1:-15}"
LOG="/tmp/raccoonclaw_oss_refresh.log"
PIDFILE="/tmp/raccoonclaw_oss_refresh.pid"
...
BASE_URL="${BASE_URL:-http://localhost:7891}"

echo "RaccoonClaw-OSS 数据刷新循环启动 (PID=$$)"
...
while true; do
  ...
  if (( SCAN_COUNTER >= SCAN_INTERVAL )); then
    SCAN_COUNTER=0
    curl -s -X POST "$BASE_URL/api/scheduler-scan" \
