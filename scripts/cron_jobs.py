...
def reset_recurring_job_waiting(job_id: str, *, now_ms: int | None = None) -> dict[str, Any]:
    """Move a recurring job back to its next future run after one cycle completes."""
    ensure_cron_dirs()
    now_ms = int(now_ms or _now_ms())
    snapshot: dict[str, Any] = {}
    ...
    def modifier(payload: dict[str, Any]) -> dict[str, Any]:
        ...
        for job in jobs:
            if not isinstance(job, dict) or str(job.get("id") or "") != job_id:
                continue
            schedule = job.get("schedule") if isinstance(job.get("schedule"), dict) else {}
            if str(schedule.get("kind") or "").strip().lower() != "recurring":
                snapshot.update(copy.deepcopy(job))
                break
            state = job.get("state") if isinstance(job.get("state"), dict) else {}
            state["running"] = False
            state["nextRunAtMs"] = compute_next_run_at_ms(schedule, now_ms=now_ms, first_run=False)
            if not str(state.get("lastRunStatus") or "").strip():
                state["lastRunStatus"] = "pending"
            if not str(state.get("lastDeliveryStatus") or "").strip():
