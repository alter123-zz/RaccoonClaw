...
def main():
    ...
    for t in tasks:
        ...
        # Prefer deliverable file path over raw output text
        if output_artifacts:
            best_artifact = output_artifacts[0]
            resolved_output = str(best_artifact.get('path') or '')
        elif raw_output and pathlib.Path(raw_output).exists():
