...
Usage:
  python3 scripts/skill_manager.py add-remote --agent planning --name code_review \\
    --source https://raw.githubusercontent.com/org/skills/main/code_review/SKILL.md \\
    --description "代码审查"
  
  python3 scripts/skill_manager.py list-remote
  
  python3 scripts/skill_manager.py update-remote --agent planning --name code_review
  
  python3 scripts/skill_manager.py remove-remote --agent planning --name code_review
  
  python3 scripts/skill_manager.py import-official-hub --agents planning,review_control,delivery_ops
"""
import sys
import json
import pathlib
...
SKILL_AGENT_MAPPING = {
    'code_review': ('engineering', 'compliance_test', 'review_control'),
    'api_design': ('engineering', 'review_control'),
    'security_audit': ('secops', 'review_control'),
    'data_analysis': ('business_analysis', 'review_control'),
    'doc_generation': ('brand_content', 'review_control'),
    'test_framework': ('engineering', 'compliance_test', 'review_control'),
}
...
def import_official_hub(agent_ids: list) -> bool:
    ...
    for skill_name, url in OFFICIAL_SKILLS_HUB.items():
        ...
        if not agent_ids:
            target_agents = SKILL_AGENT_MAPPING.get(skill_name, ['review_control'])
...
def main():
    parser = argparse.ArgumentParser(description='现代公司架构 Skill 管理工具',
