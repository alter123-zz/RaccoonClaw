...
from cron_jobs import CRON_JOBS_PATH, claim_due_jobs, finalize_job_run, set_job_enabled, sync_jobs_from_tasks, upsert_job_for_task
...
# When OpenClaw Gateway is offline, auto-advance tasks through the pipeline
# so the system can demonstrate full end-to-end delivery without agents.
DEMO_MODE = os.environ.get('RACCOONCLAW_DEMO', '').strip().lower() in ('1', 'true', 'yes')
_DEMO_STATE_DELAY = {
    'ChiefOfStaff': 8,
    'Planning': 15,
    'ReviewControl': 10,
...
_DEMO_AGENT_PROGRESS = {
    'ChiefOfStaff': '总裁办已完成分诊，转交产品规划部',
    'Planning': '产品规划部已完成方案设计，提交评审',
    'ReviewControl': '评审质控部审核通过，转交交付运营部',
    'Assigned': '交付运营部已完成派发，专项团队开始执行',
    'Doing': '专项团队执行完毕，进入交付复核',
    'Review': '交付复核完成，任务交付成功',
    'Pending': '任务已受理，进入总裁办分诊',
}
_DEMO_AGENT_OUTPUTS = {
    'chief_of_staff': '分诊完成：任务已分配至产品规划部进行方案设计。',
    'planning': '方案已产出：已制定详细执行方案，包含时间线和资源分配。',
    'review_control': '评审通过：方案符合标准，质量达标，批准执行。',
    'delivery_ops': '派发完成：任务已分发至专项团队，执行进度跟踪中。',
}
_DEMO_TODO_TEMPLATES = {
    'Planning': [
        {'id': 'p1', 'title': '需求分析与调研', 'status': 'done'},
        {'id': 'p2', 'title': '方案设计与评审材料准备', 'status': 'done'},
        {'id': 'p3', 'title': '资源与时间线规划', 'status': 'done'},
    ],
    'Doing': [
        {'id': 'd1', 'title': '环境准备与依赖检查', 'status': 'done'},
        {'id': 'd2', 'title': '核心功能实现', 'status': 'done'},
        {'id': 'd3', 'title': '测试与验证', 'status': 'done'},
        {'id': 'd4', 'title': '文档与交付物整理', 'status': 'in_progress'},
    ],
}
...
def save_tasks(tasks, data_root=None):
    ...
    if data_root is not None:
        return
    # Sync chat task completion notifications
    _notify_chat_completed_tasks(tasks)
...
# Track which task completions have already been notified
_chat_notified_done = set()
_chat_notified_progress = set()  # (taskId, state) pairs already notified


def _broadcast_notice_to_all_sessions(notice, task_id, route_mode):
    """Push a notice to all chat sessions (used for edict-board-created tasks without chatSessionId)."""
    with _CHAT_LOCK:
        ...
        for session in sessions:
            # Skip if this session already received this task's completion notice
            if session.get('id') in _chat_notified_done_sessions.get(task_id, set()):
                continue
            session.setdefault('messages', []).append({
                ...
                'content': notice,
                'createdAt': now_iso(),
                'meta': {'taskId': task_id, 'routeMode': route_mode},
            })
            session['updatedAt'] = now_iso()
            _chat_notified_done_sessions.setdefault(task_id, set()).add(session.get('id'))
        _write_chat_store(store)
        log.info(f'📢 已广播通知到 {len(sessions)} 个会话: {notice[:80]}')


_chat_notified_done_sessions: dict[str, set[str]] = {}


def _notify_chat_completed_tasks(tasks):
    """When a task reaches terminal state or changes state, push chat notification."""
    for task in tasks:
        task_id = str(task.get('id') or '')
        state = str(task.get('state') or '')
        if not task_id:
            continue
        # Skip archived tasks — they should not generate notifications
        ...
        for payload in (source_meta, template_params):
            val = str(payload.get('chatSessionId') or '').strip()
            if val:
                chat_session_id = val
                break
        ...
        if state in _TERMINAL_STATES:
            if task_id in _chat_notified_done:
                continue
            output = str(task.get('output') or '').strip()
            lines = [f'任务 {task_id} 已完成。', f'事项：{title}']
            if output:
                lines.append(f'交付物：{output}')
            notice = '\n'.join(lines)
            # Push to the associated chat session, or broadcast to all sessions if none
            if chat_session_id:
                _chat_push_notice(chat_session_id, notice, task_id, 'task_completion')
            else:
                _broadcast_notice_to_all_sessions(notice, task_id, 'task_completion')
            _chat_notified_done.add(task_id)
            continue

        # ── Intermediate state progress notification ──
        progress_key = (task_id, state)
        if progress_key in _chat_notified_progress:
            continue
        state_label = _STATE_LABELS.get(state, state)
        now_text = str(task.get('now') or '').strip()[:100]
        if now_text:
            notice = f'任务 {task_id} 状态更新：{state_label}\n{now_text}'
        else:
            notice = f'任务 {task_id} 已进入「{state_label}」阶段'
        if chat_session_id:
            _chat_push_notice(chat_session_id, notice, task_id, 'task_progress')
        _chat_notified_progress.add(progress_key)


def _chat_push_notice(chat_session_id, notice, task_id, route_mode):
    with _CHAT_LOCK:
        store = _read_chat_store()
        session = next((s for s in store.get('sessions', []) if s.get('id') == chat_session_id), None)
        if session:
            session.setdefault('messages', []).append({
                'id': f'msg-{_uuid.uuid4().hex[:12]}',
                'role': 'assistant',
            ...
            session['updatedAt'] = now_iso()
            _write_chat_store(store)
            log.info(f'📢 已推送通知到会话 {chat_session_id}: {notice[:80]}')
...
# ══ Demo 模式：自动推进任务 ══
_demo_locks = {}
_demo_locks_mutex = threading.Lock()


def _get_demo_lock(task_id):
    with _demo_locks_mutex:
        if task_id not in _demo_locks:
            _demo_locks[task_id] = threading.Lock()
        return _demo_locks[task_id]


def _demo_advance_task(task_id, current_state, data_root):
    """Demo 模式下自动推进任务到下一状态，模拟 Agent 工作。"""
    import time as _time

    normalized = _normalize_state_name(current_state)
    if normalized not in _STATE_FLOW:
        log.info(f'[demo] {task_id} 状态 {current_state} 无后续流程，停止自动推进')
        return

    next_state, from_dept, to_dept, default_remark = _STATE_FLOW[normalized]
    delay = _DEMO_STATE_DELAY.get(normalized, 10)

    log.info(f'[demo] {task_id} 等待 {delay}s 后从 {normalized} 推进到 {next_state}')
    _time.sleep(delay)

    tasks = load_tasks(data_root)
    task = next((t for t in tasks if t.get('id') == task_id), None)
    if not task:
        log.warning(f'[demo] {task_id} 已消失，跳过推进')
        return
    # Re-check state hasn't been changed externally
    if _normalize_state_name(task.get('state', '')) != normalized:
        log.info(f'[demo] {task_id} 状态已被外部修改为 {task.get("state")}，跳过自动推进')
        return

    # Add progress entry
    progress_text = _DEMO_AGENT_PROGRESS.get(normalized, f'{normalized} 阶段完成')
    progress_list = task.setdefault('progress', [])
    progress_list.append({
        'at': now_iso(),
        'agent': _STATE_AGENT_MAP.get(normalized, 'demo-agent'),
        'content': progress_text,
    })

    # Add todos for certain states
    if normalized in _DEMO_TODO_TEMPLATES:
        task['todos'] = _DEMO_TODO_TEMPLATES[normalized]

    # Add "now" text (agent output area)
    if normalized in _DEMO_AGENT_OUTPUTS:
        task['now'] = _DEMO_AGENT_OUTPUTS[normalized]
    ...
    sched['lastDispatchStatus'] = 'demo-success'
    sched['lastDispatchAgent'] = _STATE_AGENT_MAP.get(normalized, 'demo')
    sched['lastDispatchAt'] = now_iso()

    task['updatedAt'] = now_iso()
    save_tasks(tasks, data_root)
    log.info(f'[demo] ✅ {task_id} 已自动推进: {normalized} → {next_state}')

    # Continue to next state if not terminal
    if next_state not in _TERMINAL_STATES:
        threading.Thread(
            target=_demo_advance_task,
            args=(task_id, next_state, data_root),
            daemon=True,
        ).start()
...
def handle_scheduler_scan(threshold_sec=180):
    ...
    for task in tasks:
        ...
        # Demo mode: skip all scheduler enforcement
        if DEMO_MODE:
            sched_check = task.get('_scheduler', {})
            dispatch_status = str((sched_check or {}).get('lastDispatchStatus') or '')
            if dispatch_status.startswith('demo'):
                sched_check['lastProgressAt'] = now_iso()
                sched_check['stallSince'] = None
                changed = True
            continue
        source_meta = task.get('sourceMeta') or {}
        template_params = task.get('templateParams') or {}
        ...
        # ── Guard: if flow_log already shows completion, mark Done instead of retry loop ──
        flow_remarks = ' '.join(
            str(entry.get('remark') or '') for entry in task.get('flow_log') or []
        )
        if re.search(r'(✅\s*(完成|回传|复核通过|交付|已归档)|已完成.*归档|任务完成)', flow_remarks):
            task['state'] = 'Done'
            task['now'] = '任务已完成（调度器检测到历史完成记录，自动收尾）'
            task['block'] = '无'
            task['archived'] = True
            task['updatedAt'] = now_iso()
            _scheduler_mark_progress(task, '检测到已完成标记，自动收尾为 Done')
            task.setdefault('flow_log', []).append({
                'at': now_iso(),
                'from': '调度器',
                'to': '需求方',
                'remark': '✅ 调度器自动收尾：flow_log 已包含完成记录，标记为 Done 并归档'
            })
            actions.append({'taskId': task_id, 'action': 'auto-done', 'reason': 'flow_log 已包含完成标记'})
            changed = True
            continue
        ...
        # Demo mode: skip scheduler enforcement for tasks being auto-advanced
        dispatch_status = str(sched.get('lastDispatchStatus') or '')
        if dispatch_status.startswith('demo'):
            sched['lastProgressAt'] = now_iso()
            sched['stallSince'] = None
            changed = True
...
def dispatch_for_state(task_id, task, new_state, trigger='state-transition'):
    ...
    def _do_dispatch():
        try:
            if DEMO_MODE or not _check_gateway_alive():
                # Demo mode or Gateway offline: auto-advance through pipeline
                log.info(f'[demo] 🎭 {task_id} 启动 Demo 自动推进模式 (gateway={"online" if _check_gateway_alive() else "offline"})')
                _update_task_scheduler(task_id, lambda t, s: (
                    s.update({
                        'lastDispatchAt': now_iso(),
                        'lastDispatchStatus': 'demo-queued',
                        'lastDispatchAgent': runtime_agent_id,
                        'lastDispatchTrigger': trigger,
                        'lastDispatchError': '',
                    }),
                    _scheduler_add_flow(t, f'🎭 [Demo] 自动推进启动：{runtime_agent_id}（{trigger}）', to=_STATE_LABELS.get(normalized_state, normalized_state))
                ), data_root)
                threading.Thread(
                    target=_demo_advance_task,
                    args=(task_id, new_state, data_root),
                    daemon=True,
                ).start()
                return
            max_retries = 2
...
# ══ Toolbox / Bootstrap / Startup 状态 (供前端网关设置页) ══
...
def _find_openclaw_bin():
    ...
    if explicit and pathlib.Path(explicit).is_file():
        return explicit
    import shutil
    return shutil.which('openclaw') or shutil.which('openclaw') or ''


def _run_openclaw_cmd(args, timeout=20):
    """运行 openclaw 子命令并返回 (ok, output)。"""
...
# ---------------------------------------------------------------------------
# IM Channels — configuration storage and status
# ---------------------------------------------------------------------------
_IM_CHANNELS_PATH = DATA / 'im_channels.json'

_IM_CHANNEL_DEFS = {
    'feishu': {'label': '飞书', 'description': '飞书机器人', 'icon': 'feishu', 'setupMode': 'oauth'},
    'wecom': {'label': '企业微信', 'description': '企业微信应用', 'icon': 'wecom', 'setupMode': 'oauth'},
    'dingtalk': {'label': '钉钉', 'description': '钉钉机器人', 'icon': 'dingtalk', 'setupMode': 'oauth'},
    'qqbot': {'label': 'QQ 机器人', 'description': 'QQ 频道机器人', 'icon': 'qq', 'setupMode': 'token'},
    'weixin': {'label': '微信公众号', 'description': '微信公众号消息推送', 'icon': 'weixin', 'setupMode': 'token'},
}


def _read_im_channels() -> dict:
    return read_json(_IM_CHANNELS_PATH, {'channels': {}})


def _write_im_channels(store: dict):
    atomic_json_write(_IM_CHANNELS_PATH, store)


def handle_im_channels_status():
    """GET /api/im-channels/status"""
    store = _read_im_channels()
    saved = store.get('channels') or {}
    channels = []
    for key, defn in _IM_CHANNEL_DEFS.items():
        entry = saved.get(key) or {}
        configured = bool(entry.get('config'))
        ...
        channels.append({
            ...
            'enabled': enabled,
            'status': status,
            'statusLabel': {'configured': '已配置', 'draft': '未配置', 'disabled': '已禁用', 'error': '异常'}.get(status, status),
            'setupMode': entry.get('setupMode') or defn['setupMode'],
            'summary': entry.get('summary') or ('已启用' if enabled else '未配置'),
            'lastUpdated': entry.get('lastUpdated') or '',
            'configSummary': entry.get('configSummary'),
    ...
    return {
        ...
        'checkedAt': now_iso(),
        'channels': channels,
        'configuredCount': sum(1 for c in channels if c['configured']),
    }


def handle_im_channels_upsert(channel_key, config, enabled=None, setup_mode=None):
    """POST /api/im-channels/upsert"""
    if channel_key not in _IM_CHANNEL_DEFS:
        return {'ok': False, 'error': f'unknown channel: {channel_key}'}
    store = _read_im_channels()
    ...
    entry['configSummary'] = summary_parts[:5] if summary_parts else None
    _write_im_channels(store)
    result = handle_im_channels_status()
    channel = next((c for c in result['channels'] if c['key'] == channel_key), None)
    return {'ok': True, 'channel': channel}


def handle_im_channels_toggle(channel_key, enabled):
    """POST /api/im-channels/toggle"""
    if channel_key not in _IM_CHANNEL_DEFS:
        return {'ok': False, 'error': f'unknown channel: {channel_key}'}
    store = _read_im_channels()
    channels = store.setdefault('channels', {})
...
def handle_im_channels_delete(channel_key):
    """POST /api/im-channels/delete"""
    if channel_key not in _IM_CHANNEL_DEFS:
        return {'ok': False, 'error': f'unknown channel: {channel_key}'}
    store = _read_im_channels()
    ...
    if channel_key in channels:
        ...
        _write_im_channels(store)
    return {'ok': True, 'deleted': channel_key}


def handle_im_channels_test(channel_key, config):
    """POST /api/im-channels/test"""
    if channel_key not in _IM_CHANNEL_DEFS:
        return {'ok': False, 'error': f'unknown channel: {channel_key}'}
    checks = []
    # Basic validation checks per channel type
    required_fields = {
    ...
    for field in required_fields.get(channel_key, []):
        val = str(config.get(field) or '').strip()
        checks.append({
            'key': field,
            'label': field,
            'ok': bool(val),
            'detail': '' if val else f'{field} 不能为空',
        })
    all_ok = all(c['ok'] for c in checks) if checks else True
    if all_ok:
        checks.append({'key': 'format', 'label': '格式校验', 'ok': True, 'detail': '配置格式正确'})
    return {'ok': all_ok, 'checks': checks}


def handle_toolbox_status():
    """GET /api/toolbox/status — 网关/同步/配置状态。"""
    ok, output = _run_openclaw_cmd(['gateway', 'status'], timeout=20)
    gateway = {
        'ok': ok,
        'action': 'openclaw gateway status',
        'message': '执行成功' if ok else '执行失败',
        'stdout': output[:4000],
        'stderr': '',
        'code': 0 if ok else 1,
    }
    sync_status_path = DATA / 'sync_status.json'
    sync_status = read_json(sync_status_path, {})
    runtime_sync_ok = bool(sync_status.get('ok')) if 'ok' in sync_status else sync_status_path.exists()
    agent_config_ok = (DATA / 'agent_config.json').exists()
    live_status_ok = (DATA / 'live_status.json').exists()
    return {
        'ok': True,
        'gateway': gateway,
        'doctor': {'ok': True, 'message': '未执行（不阻塞）', 'action': 'openclaw doctor', 'stdout': '', 'stderr': '', 'code': None},
        'runtimeSync': {'ok': runtime_sync_ok, 'message': 'Runtime 已同步' if runtime_sync_ok else '等待同步 Runtime', 'action': 'sync_from_openclaw_runtime', 'stdout': '', 'stderr': '', 'code': 0 if runtime_sync_ok else None},
        'syncAgentConfig': {'ok': agent_config_ok, 'message': 'Agent 配置已同步' if agent_config_ok else '等待同步 Agent 配置', 'action': 'sync_agent_config', 'stdout': '', 'stderr': '', 'code': 0 if agent_config_ok else None},
        'refreshLiveStatus': {'ok': live_status_ok, 'message': '任务状态已刷新' if live_status_ok else '等待刷新任务状态', 'action': 'refresh_live_status', 'stdout': '', 'stderr': '', 'code': 0 if live_status_ok else None},
        'checkedAt': datetime.datetime.now().isoformat(timespec='seconds'),
    }


def handle_bootstrap_status():
    """GET /api/bootstrap-status — OpenClaw 环境就绪状态。"""
    cli_bin = _find_openclaw_bin()
    cfg = _read_openclaw_cfg()
    cfg_exists = _openclaw_config_path().exists()
    agents_list = (cfg.get('agents') or {}).get('list') or []

    # Check required agents from agent_registry
    agent_depts = dashboard_agent_depts()
    required_agents = [a['id'] for a in agent_depts if isinstance(a, dict) and a.get('id')]
    agent_map = {str(item.get('id')): item for item in agents_list if isinstance(item, dict)}
    missing_agents = [a for a in required_agents if a not in agent_map]
    ...
    for agent_id in required_agents:
        ...
        elif not (ws / 'SOUL.md').exists():
            missing_soul.append(agent_id)

    # Check runtime agent ids
    runtime_agent_ids = []
    if cli_bin:
        try:
            r = subprocess.run([cli_bin, 'agents', 'list', '--json'], capture_output=True, text=True, timeout=15)
            if r.returncode == 0:
                # stdout may contain multiple JSON objects or warnings; extract first valid JSON
                stdout = r.stdout.strip()
                # Try to find JSON array start
                idx = stdout.find('[')
                if idx >= 0:
                    decoder = json.JSONDecoder()
                    agent_data, _ = decoder.raw_decode(stdout[idx:])
                    runtime_agent_ids = [a.get('id', '') for a in (agent_data if isinstance(agent_data, list) else [])]
        except Exception:
            pass

    chief_runtime_ready = 'chief_of_staff' in runtime_agent_ids

    missing_total = len(missing_agents) + len(missing_workspaces) + len(missing_soul)
    ...
    return {
        'ok': True,
        'ready': missing_total == 0 and chief_runtime_ready and cfg_exists,
        'summary': summary,
        'recommendedAction': recommended_action,
        'cliInstalled': bool(cli_bin),
        'cliPath': cli_bin or '',
        'configExists': cfg_exists,
        'openclawHome': str(OCLAW_HOME),
        'runtimeAgentIds': runtime_agent_ids,
        'chiefOfStaffRuntimeReady': chief_runtime_ready,
...
def handle_desktop_startup_status():
    """GET /api/desktop/startup-status — 桌面端启动就绪状态。"""
    bootstrap = handle_bootstrap_status()
    gw_ok, gw_output = _run_openclaw_cmd(['gateway', 'status'], timeout=20)

    gateway_dashboard_url = ''
    for line in (gw_output or '').splitlines():
        l = line.strip()
        if l.lower().startswith('dashboard:'):
            gateway_dashboard_url = l.split(':', 1)[1].strip()
            break

    ready = bool(bootstrap.get('ready') and bootstrap.get('cliInstalled') and gw_ok)
    ...
    if not bootstrap.get('cliInstalled'):
        ...
        summary = 'Gateway 未就绪'
        recommended_action = 'restart_gateway'
    ...
    return {
        ...
        'ready': ready,
        'summary': summary,
        'detail': 'OpenClaw CLI、gateway 和组织配置都已通过检查，可以进入工作台。' if ready else '',
        'recommendedAction': recommended_action,
        'cliInstalled': bool(bootstrap.get('cliInstalled')),
        'cliPath': bootstrap.get('cliPath', ''),
        'gatewayStatusOk': gw_ok,
        'gatewayReachable': gw_ok,
        'gatewayDashboardUrl': gateway_dashboard_url,
        'statusOutput': gw_output[:2000] if gw_output else '',
...
def handle_bootstrap_provision():
    """POST /api/bootstrap/provision — 执行组织配置导入。"""
    cli_bin = _find_openclaw_bin()
    if not cli_bin:
        return {'ok': False, 'message': '未检测到 OpenClaw CLI，无法执行导入'}
    try:
        r = subprocess.run([cli_bin, 'gateway', 'restart'], capture_output=True, text=True, timeout=90)
        return {'ok': r.returncode == 0, 'message': 'Gateway 已重启' if r.returncode == 0 else f'重启失败: {(r.stderr or "")[:200]}'}
    except Exception as e:
...
def _chat_store_path():
    return DATA / 'chat_sessions.json'
...
def _write_chat_store(payload):
    p = _chat_store_path()
    p.parent.mkdir(parents=True, exist_ok=True)
    atomic_json_write(p, payload)


def _chat_session_summary(session):
...
def _chat_session_detail(session):
    detail = dict(session)
...
def handle_list_chat_sessions():
    with _CHAT_LOCK:
        store = _read_chat_store()
    sessions = store.get('sessions', [])
    ordered = sorted(sessions, key=lambda item: str(item.get('updatedAt') or ''), reverse=True)
    return {'ok': True, 'sessions': [_chat_session_summary(item) for item in ordered], 'count': len(ordered)}


def handle_get_chat_session(session_id):
    with _CHAT_LOCK:
        store = _read_chat_store()
...
def handle_create_chat_session(title=''):
    now = now_iso()
    session = {
...
def _append_assistant_message(session_id, assistant_text, meta=None):
    """Append an assistant message to a chat session."""
    with _CHAT_LOCK:
        store = _read_chat_store()
        session = next((item for item in store.get('sessions', []) if item.get('id') == session_id), None)
        if session:
            msg = {
                'id': f'msg-{_uuid.uuid4().hex[:12]}',
                'role': 'assistant',
...
def handle_send_chat_message(session_id, content=''):
    ...
    with _CHAT_LOCK:
        ...
        if not session:
            return {'ok': False, 'error': f'未找到会话: {session_id}'}
        pending = session.get('pendingAttachments') if isinstance(session.get('pendingAttachments'), list) else []
        if not content and not pending:
            return {'ok': False, 'error': '消息不能为空'}
        now = now_iso()
        if not session.get('messages'):
            compact = " ".join(content.split())
            session['title'] = (compact[:24] + ('…' if len(compact) > 24 else '')) or '新对话'
        session.setdefault('messages', []).append({
            'id': f'msg-{_uuid.uuid4().hex[:12]}',
            'role': 'user',
            'content': content,
    ...
    # ── Step 1: Council analysis for message routing ──
    council = None
    try:
        ...
        analyze_fn = getattr(council_module, 'analyze_with_council', None)
        if callable(analyze_fn):
            council = analyze_fn(content)
    except Exception as e:
        log.warning(f'⚠️ council 分诊异常: {e}')

    council_classification = str((council or {}).get('classification') or '').strip()
    council_flow_mode = str((council or {}).get('flowMode') or '').strip()
    should_create_task = bool(council) and council_classification not in {'', 'direct_reply'} and council_flow_mode in {'light', 'full', 'direct'}

    # ── Step 1.5: direct_reply → use council's replyText, skip agent ──
    if council_classification == 'direct_reply':
        reply_text = str((council or {}).get('replyText') or '').strip()
        if not reply_text:
            reply_text = '好的～'
        _append_assistant_message(session_id, reply_text, {})
        with _CHAT_LOCK:
            store = _read_chat_store()
            session = next((item for item in store.get('sessions', []) if item.get('id') == session_id), None)
            return {'ok': True, 'session': _chat_session_detail(session)} if session else {'ok': False, 'error': '会话已丢失'}

    # ── Step 2: Create task if council says it's substantive ──
    if should_create_task:
        dispatch_org = str((council or {}).get('dispatchOrg') or (council or {}).get('recommendedOrg') or '总裁办').strip()
        target_dept = str((council or {}).get('recommendedOrg') or dispatch_org).strip()
        official = _official_for_org(dispatch_org if council_flow_mode == 'full' else target_dept or dispatch_org)
        title = str((council or {}).get('titleHint') or '').strip() or content[:80].strip()
        if len(title) < 10:
        ...
        task_result = handle_create_task(
            ...
            params={
                ...
                'rawRequest': content,
                'userBrief': content[:6000],
                'flowMode': council_flow_mode,
                'dispatchOrg': dispatch_org,
                'dispatchAgent': str((council or {}).get('dispatchAgent') or '').strip(),
                'requiredStages': list((council or {}).get('requiredStages') or []),
            },
        ...
        if task_result.get('ok'):
            ...
            if council_flow_mode == 'direct':
                assistant_text = f'已收到需求，已创建任务 {task_id}。\n当前由总裁办按直办流程处理，直接安排给{dispatch_org or "总裁办"}执行。'
            elif council_flow_mode == 'light':
                assistant_text = f'已收到需求，已创建任务 {task_id}。\n当前由总裁办按轻流程直派至{dispatch_org}执行，跳过产品规划部与评审质控部。'
            else:
                assistant_text = f'已收到需求，已创建任务 {task_id}。\n当前将由总裁办按完整流程推进，下一步交给产品规划部处理。'
            meta = {
                'taskId': task_id,
                'routeMode': council_classification or 'create_task',
                'flowMode': council_flow_mode,
                'dispatchOrg': dispatch_org,
            }
        else:
            task_error = str(task_result.get('error') or task_result.get('message') or '任务创建失败')
            fallback_markers = ('标题过短', '不是有效任务', '任务标题不能为空', '不像是正式任务')
            if any(m in task_error for m in fallback_markers):
                # Fallback: try direct agent reply
                assistant_text = ''
            else:
                assistant_text = f'已收到需求，但创建任务失败：{task_error}'

    # ── Step 3: If no task created, try direct agent reply via openclaw ──
    if not assistant_text:
        ...
        if cli_bin:
            try:
                ...
                msg = content[:500]
                r = subprocess.run(
                    [cli_bin, 'agent', '--agent', agent_id, '-m', msg, '--timeout', '120'],
                    capture_output=True, text=True, timeout=130,
                )
                if r.returncode == 0 and r.stdout.strip():
                    # Try to extract JSON payload
                    raw = r.stdout.strip()
                    # Filter out plugin registration / diagnostic log lines
                    raw = '\n'.join(
                        line for line in raw.split('\n')
                        if not re.match(r'^(\[plugins\]|\[wecom\]|\[diagnostic\]|\[model-fallback)', line)
                        and 'Registered' not in line
                        and 'loaded' not in line.split(']')[-1].strip().lower().replace(' ', '')
                    ).strip()
                    try:
                        ...
                        pass
                    assistant_text = raw[:2000]
            except Exception as e:
                log.warning(f'⚠️ agent 回复异常: {e}')

    if not assistant_text:
        assistant_text = f'已收到：「{content[:40]}」，正在处理中。'
    ...
    with _CHAT_LOCK:
        ...
        if session:
            ...
            if meta:
                reply['meta'] = meta
            session.setdefault('messages', []).append(reply)
            session['updatedAt'] = now_iso()
            _write_chat_store(store)
            return {'ok': True, 'session': _chat_session_detail(session)}
    return {'ok': False, 'error': '会话已丢失'}


def _official_for_org(org):
    fallback_map = {
        '总裁办': 'Chief of Staff', '产品规划部': '产品规划负责人',
        '评审质控部': '评审质控负责人', '交付运营部': '交付运营负责人',
        '品牌内容部': '品牌内容负责人', '经营分析部': '经营分析负责人',
        '安全运维部': '安全运维负责人', '合规测试部': '合规测试负责人',
        '工程研发部': '工程研发负责人', '人力组织部': '人力组织负责人',
    }
    return fallback_map.get(str(org or '').strip(), '负责人')


def handle_upload_chat_attachments(session_id, files_payload):
    """Process uploaded attachments for a chat session."""
    ...
    with _CHAT_LOCK:
        ...
        if not session:
            return {'ok': False, 'error': f'未找到会话: {session_id}'}
        attachments = session.setdefault('pendingAttachments', [])
        for item in files_payload:
            filename = str(item.get('filename') or 'attachment')
            content_type = str(item.get('contentType') or '')
            raw = item.get('content')
            if isinstance(raw, (bytes, bytearray)):
                save_dir = DATA / 'chat_uploads' / session_id
                save_dir.mkdir(parents=True, exist_ok=True)
                safe_name = re.sub(r'[^A-Za-z0-9._-]+', '_', pathlib.Path(filename).stem) + pathlib.Path(filename).suffix
                save_path = save_dir / safe_name
                save_path.write_bytes(raw)
                ext = pathlib.Path(filename).suffix.lower()
                kind = 'image' if content_type.startswith('image/') or ext in {'.png', '.jpg', '.jpeg', '.gif', '.webp'} else 'document'
                attachments.append({
                    'name': filename,
                    'kind': kind,
                    'path': str(save_path),
...
def handle_remove_chat_attachment(session_id, attachment_id):
    with _CHAT_LOCK:
        store = _read_chat_store()
        session = next((item for item in store.get('sessions', []) if item.get('id') == session_id), None)
        if not session:
            return {'ok': False, 'error': f'未找到会话: {session_id}'}
        pending = session.get('pendingAttachments') if isinstance(session.get('pendingAttachments'), list) else []
        session['pendingAttachments'] = [a for a in pending if a.get('name') != attachment_id]
        _write_chat_store(store)
        return {'ok': True, 'session': _chat_session_detail(session)}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        elif p == '/api/available-skills':
            self.send_json(get_available_skills_catalog())
        ...
        elif p == '/api/toolbox/status':
            self.send_json(handle_toolbox_status())
        elif p == '/api/im-channels/status':
            self.send_json(handle_im_channels_status())
        elif p == '/api/bootstrap-status':
            self.send_json(handle_bootstrap_status())
        elif p == '/api/desktop/startup-status':
            self.send_json(handle_desktop_startup_status())
        elif p == '/api/chat/sessions':
            self.send_json(handle_list_chat_sessions())
        elif p.startswith('/api/chat/sessions/') and p.count('/') == 4:
            # GET /api/chat/sessions/{sessionId}
            session_id = p.replace('/api/chat/sessions/', '')
            self.send_json(handle_get_chat_session(session_id))
    ...
    def do_POST(self):
        ...
        if p == '/api/bootstrap/provision':
            self.send_json(handle_bootstrap_provision())
            return

        if p == '/api/open-path':
            target = pathlib.Path(str(body.get('path') or '').strip()).expanduser()
            if not str(target).strip():
                self.send_json({'ok': False, 'error': 'path 不能为空'})
            elif not target.exists():
                self.send_json({'ok': False, 'error': f'路径不存在: {target}'})
            else:
                try:
                    subprocess.Popen(['open', str(target)])
                    self.send_json({'ok': True, 'message': f'已打开: {target}'})
                except Exception as e:
        ...
        if p == '/api/chat/sessions':
            self.send_json(handle_create_chat_session(body.get('title', '')))
            return
        if p.startswith('/api/chat/sessions/') and p.endswith('/send'):
            session_id = p.replace('/api/chat/sessions/', '').replace('/send', '')
            self.send_json(handle_send_chat_message(session_id, body.get('content', '')))
            return
        if p.startswith('/api/chat/sessions/') and p.endswith('/attachments/remove'):
            session_id = p.replace('/api/chat/sessions/', '').replace('/attachments/remove', '')
            self.send_json(handle_remove_chat_attachment(session_id, body.get('attachmentId', '')))
            return
        if p.startswith('/api/chat/sessions/') and p.endswith('/attachments'):
            session_id = p.replace('/api/chat/sessions/', '').replace('/attachments', '')
                ...
                for field in form.list:
                    if field.filename:
                        files_payload.append({
                            ...
                            'contentType': field.type or '',
                            'content': field.file.read() if hasattr(field, 'file') else field.value,
                        })
            self.send_json(handle_upload_chat_attachments(session_id, files_payload))
            return

        if p == '/api/im-channels/upsert':
            self.send_json(handle_im_channels_upsert(
                str(body.get('channelKey') or ''),
                body.get('config') or {},
                enabled=body.get('enabled'),
                setup_mode=body.get('setupMode'),
            ))
            return

        if p == '/api/im-channels/toggle':
            channel_key = str(body.get('channelKey') or '').strip()
            enabled = bool(body.get('enabled'))
            self.send_json(handle_im_channels_toggle(channel_key, enabled))
            return

        if p == '/api/im-channels/delete':
            channel_key = str(body.get('channelKey') or '').strip()
            self.send_json(handle_im_channels_delete(channel_key))
            return

        if p == '/api/im-channels/test':
            self.send_json(handle_im_channels_test(
                str(body.get('channelKey') or ''),
                body.get('config') or {},
            ))
            return
...
def main():
    ...
    parser.add_argument('--demo', action='store_true', default=False, help='Demo mode: auto-advance tasks without real agents')
    args = parser.parse_args()

    if args.demo:
        global DEMO_MODE
        DEMO_MODE = True
        log.info('🎭 Demo mode enabled: tasks will auto-advance through the pipeline')
