...
import { api, AvailableSkillInfo, RemoteSkillItem } from '../api';
import { selectVisibleAgentsByMode, selectVisibleRemoteSkillsByMode } from '../workbenchSelectors';
...
function dedupeChiefOfStaffRemoteSkills(skills: RemoteSkillItem[], agents: { id: string }[]): RemoteSkillItem[] {
  const hasChiefAlias = agents.some((agent) => agent.id === 'chief_of_staff');
  if (!hasChiefAlias) return skills;
  return skills.filter((skill) => skill.agentId !== 'main');
}
...
export default function SkillsConfig() {
  ...
  useEffect(() => {
    const loadAvailableSkills = async () => {
      try {
        const r = await api.availableSkills();
        if (r.ok) setAvailableSkills(r.skills || []);
      } catch {
        // noop: 添加技能时只做辅助展示，不阻断主流程
      }
    };
    loadAvailableSkills();
  }, []);
  ...
  const pickAvailableSkill = (skill: AvailableSkillInfo) => {
    setFormData((p) => ({
      ...p,
      name: skill.name,
      desc: p.desc || skill.description || '',
    }));
  };
  ...
  const submitAddRemote = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (r.ok) {
        toast(`✅ 远程技能 ${skillName} 已添加到 ${displayAgentLabel(agentId)}`, 'ok');
  ...
  const handleQuickImport = async (skillUrl: string, skillName: string) => {
    ...
    try {
      ...
      if (r.ok) {
        toast(`✅ ${skillName} → ${displayAgentLabel(quickPickAgent)}`, 'ok');
        loadRemoteSkills();
        loadAgentConfig();
      } else {
  ...
  const visibleRemoteSkills = dedupeChiefOfStaffRemoteSkills(
    selectVisibleRemoteSkillsByMode(remoteSkills, workbenchMode),
    visibleAgents
  );
  const addTargetAgent = addForm ? visibleAgents.find((ag) => ag.id === addForm.agentId) : null;
  const currentAgentSkillNames = new Set((addTargetAgent?.skills || []).map((sk) => sk.name));
  const suggestedAvailableSkills = [...availableSkills].sort((a, b) => a.name.localeCompare(b.name, 'en'));
  const builtinAvailableSkills = suggestedAvailableSkills.filter((sk) => sk.sources.includes('builtin'));
  const existingAvailableSkills = suggestedAvailableSkills.filter((sk) =>
    sk.sources.some((source) => source === 'agent' || source === 'global')
  );
    ...
    <div>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 20, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <span style={{ fontSize: 11, color: 'var(--muted)', marginLeft: 4 }}>
          共 {visibleRemoteSkills.length} 个远程技能
        </span>
      </div>
          ...
          <div style={{ marginTop: 14, background: 'var(--panel)', border: '1px solid var(--line)', borderRadius: 12, padding: 16 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: 8 }}>
              {quickPickSource.skills.map((sk) => {
                const alreadyAdded = visibleRemoteSkills.some((r) => r.skillName === sk.name && r.agentId === quickPickAgent);
                return (
                  <div
                    key={sk.name}
                    style={{
                ...
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
                    ...
                    <span style={{ fontSize: 11, color: 'var(--muted)', background: 'var(--panel2)', padding: '2px 8px', borderRadius: 6 }}>
                      {agInfo?.emoji} {agInfo?.label || displayAgentLabel(sk.agentId)}
                    </span>
                  </div>
                  {sk.description && (
  ...
  return (
    <div>
      ...
      <div className="skills-tabs-shell">
        {[
          { key: 'local', label: '🏛️ 本地技能', count: visibleAgents.reduce((n, a) => n + (a.skills?.length || 0), 0) },
          { key: 'remote', label: '🌐 远程技能', count: visibleRemoteSkills.length },
        ].map((t) => (
          <div
            key={t.key}
            onClick={() => setActiveTab(t.key as 'local' | 'remote')}
            className={`skills-top-tab ${activeTab === t.key ? 'active' : ''}`}
          >
            {t.label}
            {t.count > 0 && (
              <span className="skills-top-tab-count">
                {t.count}
              </span>
            )}
          </div>
          ...
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              <div style={{ fontSize: 11, color: 'var(--acc)', fontWeight: 700, letterSpacing: '.04em', marginBottom: 4 }}>
                {displayAgentLabel(skillModal.agentId)}
              </div>
        ...
        <div className="modal-bg open" onClick={() => setAddForm(null)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              ...
              {!!suggestedAvailableSkills.length && (
                <div
                  style={{
                ...
                >
                  {!!builtinAvailableSkills.length && (
                    <div style={{ marginBottom: existingAvailableSkills.length ? 14 : 0 }}>
                      <div style={{ fontSize: 12, fontWeight: 700, marginBottom: 10, display: 'flex', alignItems: 'center', gap: 8 }}>
                        <span>内置技能</span>
                        <span style={{ fontSize: 10, padding: '1px 6px', borderRadius: 999, background: '#1a2040', color: 'var(--acc)' }}>
                          {builtinAvailableSkills.length}
                        </span>
                      </div>
                      <div style={{ fontSize: 11, color: 'var(--muted)', marginBottom: 10 }}>
                        来自 OSS 仓库的内置技能。已安装到当前 Agent 的技能会标记出来，不再截断显示。
                      </div>
                      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', maxHeight: 240, overflowY: 'auto', paddingRight: 4 }}>
                        {builtinAvailableSkills.map((sk) => (
                          <button
                            key={`builtin-${sk.name}`}
                            type="button"
                            onClick={() => pickAvailableSkill(sk)}
                            style={{
                              padding: '6px 10px',
                              borderRadius: 999,
                              border: '1px solid var(--line)',
                              background: formData.name === sk.name ? '#0d1f45' : 'var(--panel2)',
                              color: formData.name === sk.name ? '#eef4ff' : 'var(--text)',
                              cursor: 'pointer',
                              fontSize: 11,
                              opacity: currentAgentSkillNames.has(sk.name) ? 0.72 : 1,
                            }}
                            title={`${sk.description || '无描述'}\n${sk.path}`}
                          >
                            {sk.name}
                            {currentAgentSkillNames.has(sk.name) ? ' · 已安装' : ''}
                          </button>
                        ))}
                      </div>
                    </div>
                  )}
                  {!!existingAvailableSkills.length && (
                    <div>
                      <div style={{ fontSize: 12, fontWeight: 700, marginBottom: 10, display: 'flex', alignItems: 'center', gap: 8 }}>
                        <span>OpenClaw 已有技能</span>
                        <span style={{ fontSize: 10, padding: '1px 6px', borderRadius: 999, background: '#1a2040', color: 'var(--acc)' }}>
                          {existingAvailableSkills.length}
                        </span>
                      </div>
                      <div style={{ fontSize: 11, color: 'var(--muted)', marginBottom: 10 }}>
                        下面是当前 OpenClaw 运行时环境里已经存在的技能。已安装到当前 Agent 的技能会标记出来，不再截断显示。
                      </div>
                      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', maxHeight: 240, overflowY: 'auto', paddingRight: 4 }}>
                        {existingAvailableSkills.map((sk) => (
                          <button
                            key={`existing-${sk.name}`}
                            type="button"
                            onClick={() => pickAvailableSkill(sk)}
                            style={{
                              padding: '6px 10px',
