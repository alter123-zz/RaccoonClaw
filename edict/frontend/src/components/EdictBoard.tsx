...
export function TaskMonitorPanel({ singleRow = false }: { singleRow?: boolean }) {
  ...
  const visibleFocusFilters = publicConfig.features.scheduledTasks
    ? FOCUS_FILTERS
    : FOCUS_FILTERS.filter((item) => item.key !== 'scheduled');
...
export function TaskLaunchPanel() {
  ...
  const runningScheduledEdicts = [...scheduledEdicts]
    .filter((task) => !isAutomationTask(task) && !isTerminalState(task.state))
    .sort((a, b) => {
  ...
  const [quickTaskKind, setQuickTaskKind] = useState<(typeof TASK_KIND_OPTIONS)[number]['key']>('oneshot');
  const [quickScheduleMode, setQuickScheduleMode] = useState<(typeof RECURRING_OPTIONS)[number]['key']>('daily');
  const [quickOneShotAt, setQuickOneShotAt] = useState('');
  ...
  const visibleTaskKindOptions = publicConfig.features.scheduledTasks
    ? TASK_KIND_OPTIONS
    : TASK_KIND_OPTIONS.filter((option) => option.key !== 'recurring');
  ...
  useEffect(() => {
    if (!publicConfig.features.scheduledTasks && quickTaskKind === 'recurring') {
      setQuickTaskKind('oneshot');
    }
  }, [publicConfig.features.scheduledTasks, quickTaskKind]);

  const scheduleLabel = (() => {
    if (quickTaskKind === 'oneshot') {
      return quickOneShotAt ? `单次任务 · ${quickOneShotAt.replace('T', ' ')}` : '单次任务';
    }
  ...
  const handleQuickCreate = async (e: React.FormEvent) => {
    ...
    if (quickTaskKind === 'oneshot' && !quickOneShotAt) {
      toast('单次任务需要填写执行时间', 'err');
      return;
    }
    setCreating(true);
    try {
      const scheduleLines = [];
      if (scheduleLabel) scheduleLines.push(`任务类型：${scheduleLabel}`);
      if (quickTaskKind === 'oneshot' && quickOneShotAt) scheduleLines.push(`执行时间：${quickOneShotAt.replace('T', ' ')}`);
      if (quickTaskKind === 'recurring') {
        scheduleLines.push(`执行频率：${scheduleLabel}`);
      }
      ...
      const r = await api.createTask({
        ...
        params: {
          ...
          scheduleLabel,
          scheduledAt: quickTaskKind === 'oneshot' ? quickOneShotAt : '',
  ...
  return (
    <div className="board-shell">
      ...
      <div className="board-command-grid">
        <form className="quick-create-card" id="quick-create" onSubmit={handleQuickCreate}>
          ...
          <div className="qc-sub">
            {publicConfig.features.scheduledTasks
              ? '可发起单次任务和定时任务。执行结果仍统一回收到交付归档。'
              : '当前社区版默认开放单次任务。执行结果仍统一回收到交付归档。'}
          </div>
          ...
          <div className="qc-fields">
            ...
            <label className="qc-field">
              ...
              <select value={quickTaskKind} onChange={(e) => setQuickTaskKind(e.target.value as (typeof TASK_KIND_OPTIONS)[number]['key'])}>
                {visibleTaskKindOptions.map((option) => (
                  <option key={option.key} value={option.key}>{option.label}</option>
                ))}
              </select>
            </label>
            ...
            {quickTaskKind === 'oneshot' && (
              <label className="qc-field">
                <span>执行时间</span>
                <input type="datetime-local" value={quickOneShotAt} onChange={(e) => setQuickOneShotAt(e.target.value)} />
              </label>
            )}
          ...
          <div className="qc-actions">
            <div className="qc-hint">
              {quickTaskKind === 'oneshot'
                ? `${scheduleLabel || '单次执行时间待补充'}，到点后只执行一次，结果仍会进入交付归档。`
                : `${scheduleLabel || '定时规则待补充'}，当前先纳入统一建单与归档，后续由调度器按计划反复执行。`}
            </div>
      ...
      {publicConfig.features.scheduledTasks && scheduledEdicts.length > 0 && (
