    ...
    (async () => {
      ...
      try {
        ...
        if (nextSessions.length) {
          const initialId = nextSessions[0].id;
          setLoadingSession(true);
          try {
            const detail = await api.chatSession(initialId);
            if (cancelled) return;
            if (detail.ok && detail.session) {
              setActiveSessionId(initialId);
              setActiveSession(detail.session);
            }
          } catch {
            if (!cancelled) toast('会话加载失败', 'err');
          } finally {
        ...
        if (!cancelled) toast('对话列表加载失败', 'err');
      } finally {
