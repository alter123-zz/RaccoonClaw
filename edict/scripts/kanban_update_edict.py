#!/usr/bin/env python3
"""Task update helper for the workbench API.

It keeps a file-compatible CLI surface and writes through the REST API when
available, otherwise falling back to JSON files.
"""
...
# State → TaskState value 映射
_STATE_TO_EDICT = {
    'ChiefOfStaff': 'chief_of_staff', 'Planning': 'planning', 'ReviewControl': 'review_control',
...
def _check_api():
    ...
    if _api_ok is None:
        ...
        if _api_ok:
            log.debug('API 可用，使用 API 模式')
        else:
            log.debug('API 不可用，降级到 JSON 模式')
    return _api_ok
...
def cmd_create(task_id, title, state, org, official, remark=None):
    ...
    if _check_api():
        ...
        if result:
            log.info(f'✅ 创建 {task_id} → Task {result.get("task_id", "?")} | {title[:30]}')
            return
