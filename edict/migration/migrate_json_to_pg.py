...
读取 data/tasks_source.json，导入到 Postgres。
...
# 任务状态 → TaskState
STATE_MAP = {
    "ChiefOfStaff": TaskState.ChiefOfStaff,
    "Planning": TaskState.Planning,
    "ReviewControl": TaskState.ReviewControl,
    ...
    "Inbox": TaskState.ChiefOfStaff,
    "": TaskState.ChiefOfStaff,
}


def parse_old_task(old: dict) -> dict:
    """将 JSON 任务转换为数据库写入参数。"""
    state_str = old.get("state", "ChiefOfStaff")
    state = STATE_MAP.get(state_str, TaskState.ChiefOfStaff)
