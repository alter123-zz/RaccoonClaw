...
In this workspace, the backend dependencies have already been installed into `.venv-backend/`. On a fresh clone, install the backend stack from `Raccoon/backend/requirements.txt` before using the single-backend start script.
